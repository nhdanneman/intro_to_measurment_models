'''
Q: What do the posteriors on these models look like for b1, thresh?
...
def data_creator(N=100, n_levs=[2, 3, 4]):
    ...
    b1 = np.random.uniform(.5, 1.5, n_det)
    ...
    for j in range(n_det):
        ...
        for k in range(n_levs[j] - 2):
            ...
            next_cut = np.random.uniform(next_cut_center - .12, next_cut_center + .12)
...
samples = model.sample(10000, vars=['thresh', 'x', 'b1'])

# some checks:
x_est = np.apply_over_axes(np.mean, samples['x'], [1, 2]).flatten()
plt.scatter(x_est, x)

# A model includes info on b1 and thresholds
# Sigma and b0 are fixed
# The 'info' is REALLY a full distribution
# Hopefully summary stats of that can form a reasonable prior

# Look at and summarize b1's
samples['b1'].shape  # (5, 1000, 4)

plt.hist(samples['b1'][0,:,:].flatten())
plt.hist(samples['b1'][1,:,:].flatten())
plt.hist(samples['b1'][2,:,:].flatten())
plt.hist(samples['b1'][3,:,:].flatten())
plt.hist(samples['b1'][4])

plt.scatter(range(10000), samples['b1'][3,:,0])
plt.scatter(range(10000), samples['b1'][3,:,1])
plt.scatter(range(10000), samples['b1'][3,:,2])
plt.scatter(range(10000), samples['b1'][3,:,3])

np.mean(samples['b1'][4,:,0])
np.mean(samples['b1'][4,:,1])
np.mean(samples['b1'][4,:,2])
np.mean(samples['b1'][4,:,3])